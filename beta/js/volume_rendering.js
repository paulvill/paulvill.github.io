...
for (tstep = 0; tstep < trange; ++tstep) {
 ...
 for (var zval = 0; zval<zrange; zval++) {
 	mesh = new THREE.Mesh(new THREE.PlaneGeometry(8, 8), new THREE.MeshBasicMaterial({map:textureArrayZ[ch * imageCountZ + tstep*zrange + zval], 
 		side:THREE.DoubleSide, opacity:0.25, transparent:true, depthWrite:false, blending:THREE.AdditiveBlending}));
 	mesh.position.z = 2-0.1*zval;
